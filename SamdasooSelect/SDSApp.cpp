// SamdasooSelect.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
#include "pch.h"
...
std::shared_ptr<SDSWorker> SDSApp::worker;
...
int main()
{
    ...
    SDSApp::worker = worker;

    worker->Init();

    std::shared_ptr<SDSServer> svr = std::make_shared<SDSServer>(HOST, PORT, BACKLOG);
    svr->CreateSocket();
    svr->Listen();
